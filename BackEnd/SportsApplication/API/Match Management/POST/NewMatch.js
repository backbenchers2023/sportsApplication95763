...
router.post('/NewMatch', async (req, res) => {
  ...
  try {
    const { logo, team, user_id, venue, match_status, match_type, start_time, duration } = req.body;
    ...
    try {
      await db.collection('matches').doc("Match" + match_id.toString()).set({
        match_id,logo,team, user_id,venue,match_status,match_type,start_time,duration,timestamp
      });
